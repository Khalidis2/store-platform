...
export default async function GuestOrderPage({
  ...
  return (
    <main style={{ maxWidth: 720, margin: "0 auto", padding: "2rem 1rem" }}>
      <h1>Order status</h1>
      <p style={{ color: "#666" }}>
        Order {order.id.slice(0, 8).toUpperCase()} · {new Date(order.created_at).toLocaleDateString("en-AE")}
      </p>

      <section style={{ marginTop: "2rem" }}>
        <h2>{statusLabel(order.status)}</h2>
        {order.status === "paid" && <p>Your payment is confirmed and the store is preparing your order.</p>}
        {order.status === "shipped" && <p>Your order has been shipped.</p>}
        {order.status === "delivered" && <p>Your order has been marked as delivered.</p>}
        {order.status === "expired" && <p>This checkout expired before payment was completed.</p>}
      </section>
      ...
      <section style={{ marginTop: "2rem" }}>
        ...
        <ul style={{ paddingLeft: "1.25rem" }}>
          {order.line_items.map((item, index) => (
            <li key={`${item.name}-${index}`} style={{ marginBottom: "0.5rem" }}>
      ...
      <p style={{ marginTop: "2rem", color: "#666", fontSize: "0.9rem" }}>
        Keep this private link to check your order status again later.
      </p>
    </main>
  );
}
