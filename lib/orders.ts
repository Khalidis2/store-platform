...
export async function applyRefund(orderId: string, storeId: string, refundedAmountCents: number) {
  ...
  try {
    ...
    const { rows } = await client.query(
      "select status, total_cents, line_items, has_shipped, refunded_amount_cents from orders where id = $1 and store_id = $2 for update",
      [orderId, storeId]
    );
    ...
    if (
      !existing ||
      !["paid", "shipped", "partially_refunded"].includes(existing.status) ||
      refundedAmountCents <= existing.refunded_amount_cents
    ) {
